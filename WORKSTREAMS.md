...
- **`.github/workflows/ci.yml`** — `on: pull_request, workflow_dispatch`. Two jobs: `lint-test` (ruff + mypy + pytest) → `eval-gate` (`needs: lint-test`) running `uv run python -m app.eval --gate --min-recall 0.7 --min-citation-acc 0.8 --min-faithfulness 0.85`. Both use `astral-sh/setup-uv@v5` with cache. Wire repo secrets `ANTHROPIC_API_KEY`, `VOYAGE_API_KEY`.
